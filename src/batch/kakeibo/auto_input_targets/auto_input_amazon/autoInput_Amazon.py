...
class AutoInput_Amazon(BaseAutoInput):
    ...
    def main(self, request_dict):
        """
        処理概要: Amazon購入履歴を取り込む。
        処理内容:
          1. 対象ユーザーとログイン状態を確認する。
          2. 購入履歴を取得する。
          3. 未登録明細を小票へ変換する。
          4. 取込結果を返す。
    ...
    def fetch_recent_orders(self, session, first_response):
        ...
        Args:
            session (Any): sessionの値。
            first_response (Any): first_responseの値。
    ...
    def parse_order_date_value(value):
        """
        parse_order_date_valueの処理を実行する。
    ...
    def save_order_rows(self, rows, user_id):
        ...
        for row in rows:
            ...
            existing = self.database.select(
                self.database.read_sql("SELECT_KAKEIBO_AUTO_INPUT_CONT", location=__file__),
    ...
    def register_pending_orders(self, user_id):
        ...
        rows = self.database.select(
            self.database.read_sql("SELECT_KAKEIBO_AUTO_INPUT_CONT_02", location=__file__),
    ...
    def split_total_price(total_price, count):
        """
        split_total_priceの処理を実行する。

        Args:
            total_price (Any): total_priceの値。
            count (Any): countの値。
    ...
    def default_unknown_amazon_category_mapping(self, categories):
        """
        default_unknown_amazon_category_mappingの処理を実行する。
    ...
    def load_receipt_categories(self, user_id):
        """
        load_receipt_categoriesの処理を実行する。
    ...
    def mark_order_registered(self, row):
        ...
        self.database.update(
            self.database.read_sql("UPDATE_KAKEIBO_AUTO_INPUT_CONT", location=__file__),
    ...
    def save_challenge(self, config, user_id, challenge_id, expires_at, action_url, cookies, form_info):
        ...
        Args:
            config (Any): configの値。
            user_id (Any): ユーザーID。
            challenge_id (Any): challenge_idの値。
            expires_at (Any): expires_atの値。
            action_url (Any): action_urlの値。
            cookies (Any): cookiesの値。
            form_info (Any): form_infoの値。

        Returns:
            Any: 処理結果。
        """
        self.database.update(
            self.database.read_sql("UPDATE_AUTO_INPUT_INFO", location=__file__),
            {
    ...
    def save_authenticated_session_for_user(self, user_id, cookies):
        ...
        ymd, hms = now_ymd_hms()
        self.database.update(
            self.database.read_sql("UPDATE_AUTO_INPUT_INFO_02", location=__file__),
    ...
    def update_status(self, config, user_id, status):
        ...
        """
        self.database.update(
            self.database.read_sql("UPDATE_AUTO_INPUT_INFO_03", location=__file__),
