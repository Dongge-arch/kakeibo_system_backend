...
from src.batch.kakeibo.auto_input_targets.auto_input_suica.autoInput_Suica import (
    AutoInput_Suica,
...
class AutoLinkageBase(BaseRestApi):
    """自動連携の設定と実行に共通する処理。"""
    ...
    def find_row(self, connection_type, user_id):
        ...
        Args:
            connection_type (Any): connection_typeの値。
            user_id (Any): ユーザーID。
    ...
    def public_row(self, place, row, include_account=False):
        ...
        Args:
            place (Any): placeの値。
            row (Any): rowの値。
            include_account (Any): include_accountの値。
