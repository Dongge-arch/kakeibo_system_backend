...
def round_price(value):
    """計算された価格を日本円の1円単位で四捨五入する。"""
    return int(Decimal(str(to_number(value))).quantize(Decimal("1"), rounding=ROUND_HALF_UP))
...
def enrich_detail_prices(detail: dict, tax_flag) -> dict:
    ...
    if is_tax_excluded:
        ...
        # 印字合計が税抜小計と同額の場合だけ、別項目の税込合計を使用する。
        explicit_included = to_number(detail.get("taxIncludedTotalPrice"))
        if explicit_included and display_total == round_price(base_total):
            tax_included_total = explicit_included
        else:
            tax_included_total = display_total or explicit_included or round_price(base_total * tax_multiplier)
        tax_excluded_total = (
