...
class ReceiptExportService:
    ...
    def prepare(self, request):
        ...
        if export_type not in ("excel", "pdf"):
            return response(400, {"errorMessage": "出力形式が不正です。"})

        rows = self.normalize_rows((request or {}).get("rows") or [])
        if not rows:
            return response(400, {"errorMessage": "出力対象のデータがありません。"})
        ...
        return response(200, {"url": f"/export/receipt/page/{token}"})
...
"""
    ...
    def pdf_cover_page(self, rows, total, category_totals, month_totals):
        """
        pdf_cover_pageの処理を実行する。

        Args:
            rows (Any): rowsの値。
            total (Any): totalの値。
            category_totals (Any): category_totalsの値。
            month_totals (Any): month_totalsの値。
    ...
    def pdf_detail_page(self, rows, offset, total_count):
        """
        pdf_detail_pageの処理を実行する。

        Args:
            rows (Any): rowsの値。
            offset (Any): offsetの値。
            total_count (Any): total_countの値。
    ...
    def pdf_section_title(self, commands, x, y, title):
        """
        pdf_section_titleの処理を実行する。

        Args:
            commands (Any): commandsの値。
            x (Any): xの値。
            y (Any): yの値。
            title (Any): titleの値。
    ...
    def make_pdf(self, page_streams):
        ...
        Args:
            page_streams (Any): page_streamsの値。
    ...
    def wrap_pdf_text(self, value, max_width, size=10):
        """
        wrap_pdf_textの処理を実行する。

        Args:
            value (Any): valueの値。
            max_width (Any): max_widthの値。
            size (Any): sizeの値。
